...
EgaroucidReplaySnapshot snapshot_for(Position position, int ply, int final_disc_diff) {
  const std::string board = relative_board_a1_to_h8(position);
  return EgaroucidReplaySnapshot{
      .ply = ply,
...
EgaroucidReplayResult reject(std::string error) {
  return EgaroucidReplayResult{
      .accepted = false,
...
EgaroucidReplayResult replay_egaroucid_sequence(std::string_view text) {

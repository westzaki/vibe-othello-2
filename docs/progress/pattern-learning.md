...
| Advance 50k move-teacher growth validation | done | The local growth route now has same-source 50k seeds 0/1/2 complete against a fair exact-root v2 baseline, all three seeds positive on top1/pairwise/regret and held-out support, 30 bounded arena variants with zero failed games, and a `promote_to_larger_local_validation` scorecard. A connected-board-game 1m resplit was built from a sequence-cache hit, connected 50k seed 0 was positive with supportive bounded arena results, and a connected 100k low-empty source was selected. The next evidence step is a fair 100k exact-root v2 baseline plus 100k seed 0, not pattern-v3, a pairwise rank trainer, or an optimizer sweep. |
| Cache exact move-teacher labels | done | `materialize_move_teacher_from_cache.py` stores split-independent exact move-teacher root payloads in a local per-root cache and materializes `move-teacher.tsv` plus `child-normalized.tsv` with current record ids, split assignments, root phases, and row order. The campaign, matrix, and growth-cycle runners pass through cache reuse/write/partial-miss flags; full-hit and partial-miss reuse validate board identity, board contents, schema/semantic metadata, label kind/unit/perspective, solver version, `max_empty`, legal move rows, and cache conflicts. Synthetic smoke coverage checks full-hit remap, partial-hit failure without opt-in, partial-miss fake-generator orchestration, mismatch failures, source conflicts, semantic mismatch failures, report fields, and runner pass-through. Real local validation populated 50k seed 0 in 8.842 seconds, materialized same-source 50k seed 1 in 22.348 seconds, materialized connected 50k seed 0 in 22.296 seconds with 50,000/50,000 hits and 1,591,232,496 exact nodes avoided per materialization, probed the connected 100k source at 50,000 hits / 50,000 misses, and completed a capped 40-root real partial solve with 20 initial hits, 20 newly solved roots, 377,529 exact nodes saved, 467,555 newly solved nodes, and complete final cache materialization. |
| Derive exact root labels from move-teacher rows | done | `derive_exact_root_labels_from_move_teacher.py` derives `teacher_exact_final_disc_diff` root labels from complete move-teacher TSVs using the max root move score and sum child `teacher_nodes` aggregate, writes `apply_teacher_labels.py`-compatible TSVs, and is covered by synthetic smoke tests for max-score selection, missing-root failure, and duplicate root/move failure. The capped real partial validation derived 40 exact-root labels from the complete move-teacher TSV; the full 100k derived baseline was not built in this PR. |

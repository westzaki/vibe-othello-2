...
| Cache exact move-teacher labels | done | `materialize_move_teacher_from_cache.py` stores split-independent exact move-teacher root payloads in a local per-root cache and materializes `move-teacher.tsv` plus `child-normalized.tsv` with current record ids, split assignments, root phases, and row order. The campaign, matrix, and growth-cycle runners pass through cache reuse/write flags, full-hit reuse validates board identity, board contents, schema/semantic metadata, label kind/unit/perspective, solver version, `max_empty`, and legal move rows, and synthetic smoke coverage checks full-hit remap, mismatch failures, missing-root failures, semantic mismatch failures, report fields, and runner pass-through. Real local validation populated 50k seed 0 in 8.842 seconds, materialized same-source 50k seed 1 in 22.348 seconds, and materialized connected 50k seed 0 in 22.296 seconds with 50,000/50,000 hits and 1,591,232,496 exact nodes avoided per materialization. Partial-miss solve is intentionally left for a follow-up. |

...
def record_label(record: dict[str, Any], index: int) -> str:
    ...
    depth = record.get("depth", "<missing-depth>")
    mode = record.get("mode", "<missing-mode>")
    tt_mode = record.get("tt_mode", "<missing-tt-mode>")
    return f"record {index + 1} position={position_id} depth={depth} mode={mode} tt={tt_mode}"
...
def main() -> int:
    ...
    if args.write_normalized is not None:
        ...
        print(f"wrote normalized search golden: {args.write_normalized}")
        return 0
    ...
    if failures:
        print("search golden mismatch:", file=sys.stderr)
        for failure in failures:
            print(failure, file=sys.stderr)
        return 1
    print(f"search golden match: {len(actual_records)} records")
    return 0

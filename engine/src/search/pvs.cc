...
SearchValue pvs(SearchContext* context, Score alpha, Score beta, Depth depth, Ply ply) {
  ...
  if (frame.moves.size == 0) {
    ...
    const SearchValue child = pvs(context, static_cast<Score>(-beta), static_cast<Score>(-alpha),
  ...
  for (std::uint8_t move_index = 0; move_index < frame.moves.size; ++move_index) {
    ...
    SearchValue child{};
    if (move_index == 0) {
      child = pvs(context, static_cast<Score>(-beta), static_cast<Score>(-alpha),
                  static_cast<Depth>(depth - 1), static_cast<Ply>(ply + 1));
    } else {
      child = null_window_search(context, static_cast<Score>(-alpha), static_cast<Depth>(depth - 1),
                                 static_cast<Ply>(ply + 1));
      Score score = static_cast<Score>(-child.score);

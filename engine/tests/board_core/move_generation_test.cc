...
TEST_CASE("initial black legal moves are generated", "[board_core][move_generation]") {
...
TEST_CASE("legal moves are empty squares", "[board_core][move_generation]") {
  const Position position = initial_position();

  REQUIRE((legal_moves(position) & occupied(position)) == 0);
}

TEST_CASE("legal moves match squares that flip discs", "[board_core][move_generation]") {
...
TEST_CASE("positions without legal moves return zero", "[board_core][move_generation]") {
...
TEST_CASE("invalid positions return zero legal moves", "[board_core][move_generation]") {
...
TEST_CASE("legal move generation does not wrap across files", "[board_core][move_generation]") {
...
TEST_CASE("legal move generation works along board edges", "[board_core][move_generation]") {
...
TEST_CASE("legal move generation handles multi-disc runs", "[board_core][move_generation]") {
  constexpr Position position{
      .player = bit(square(0, 0)),
      .opponent = squares({
          square(1, 0),
...
TEST_CASE("legal move generation handles all eight directions", "[board_core][move_generation]") {
